...
export const DOCS_PAGES: Record<string, DocsPage> = {
  ...
  "use/wallet-guide": page("use", "wallet-guide", "Wallet and testnet guidance.", "DOCS / USE VERD / WALLET GUIDANCE", "Verd keeps wallet actions behind a connected account and the correct testnet network. You can inspect public facility data without connecting a wallet.", ["Networks", "Before signing", "Testnet assets", "What Verd never asks for"], [
    ...
    { kind: "bullets", items: ["Read the full action label, amount, network, and facility context before confirming a wallet prompt.", "The lender needs enough tCTC for the principal and gas. The borrower needs Sepolia ETH for gas and enough WETH for the stated reserve.", "Verd can wrap a borrower's Sepolia ETH into WETH when the reserve is short. It never wraps or moves assets without a wallet confirmation.", "When an action needs another network, Verd requests that network first, then opens the intended wallet action after you approve the switch.", "Re-read facility state after a transaction before starting another action." ] },
  ...
  "build/release": page("build", "release", "Release the reserve.", "DOCS / BUILD / RELEASE THE RESERVE", "The borrower releases the aToken position from the locker after the immutable time boundary.", ["Before release", "Release", "Completion"], [
    ...
    { kind: "callout", tone: "warning", label: "CURRENT PRODUCT BOUNDARY", text: "ReserveLocker release remains time-based and borrower-authorized. Verd sequences repayment first and records an authenticated release proof afterward, but the locker itself does not receive an automatic repayment signal from Creditcoin." },
  ]),
  ...
  "integrate/wallet-network": page("integrate", "wallet-network", "Wallet and network handling.", "DOCS / INTEGRATE / WALLET AND NETWORK", "Wallet actions are explicit, role-aware, and tied to the network that owns the next transaction.", ["CC3 actions", "Sepolia actions", "Signatures"], [
    { kind: "table", headers: ["Action", "Network", "Role"], rows: [["Create, fund, draw, repay", "Creditcoin CC3", "Lender or borrower as recorded"], ["Create locker, approve WETH, supply Aave, release reserve", "Ethereum Sepolia", "Borrower"], ["Register binding, qualification, or release job", "Wallet signature, then CC3 worker path", "Borrower"]] },
  ...
  "reference/verd-contract": page("reference", "verd-contract", "Verd contract.", "DOCS / REFERENCE / VERD CONTRACT", "Verd.sol records the facility terms, lifecycle, binding, rate, interest, proof, and repayment state on Creditcoin.", ["Reads", "Writes", "Invariants"], [
    { kind: "bullets", items: ["facilityExists, facilityState, getFacility, getFacilityStatus, getFacilityProof, getFacilityReleaseEvidence, getFacilityLockerBinding, getFacilityFinancials", "createFacility, fundFacility, drawFacility, accrueInterest, bindReserveLocker, repayFacility, recordReserveRelease", "Preferred APR changes only after proof-backed condition activation", "Repayment and release records are single-use and read back from CC3" ] },
  ...
  "reference/events": page("reference", "events", "Events.", "DOCS / REFERENCE / EVENTS", "Events make lifecycle transitions inspectable across the two execution domains.", ["Creditcoin", "Ethereum", "Evidence use"], [
    { kind: "table", headers: ["Contract", "Events"], rows: [["Verd", "FacilityCreated, ReserveLockerBound, FacilityFunded, FacilityDrawn, InterestAccrued, PreferredRateConditionActivated, FacilityRepaid, ReserveReleaseRecorded"], ["ReserveLocker", "ReserveReleased"], ["ReserveLockerFactory", "ReserveLockerCreated"]] },
