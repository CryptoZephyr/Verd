...
import { FACILITY_STATE_LABELS, PUBLISHED_COMPLETED_REFERENCE, VERIFIED_DEPLOYMENT, VERIFIED_EVIDENCE, createFacility, createReserveLocker, drawFacility, estimateRepayment, explorerAddress, explorerTx, formatApr, formatCtc, formatDate, formatToken, fundFacility, hasAddress, hasProof, readFacilityProgress, readQualificationJob, readRecentFacilities, readReserveLocker, readTestnetBalances, registerQualificationJob, releaseReserve, rememberFacility, repaymentSubmissionAmount, repayFacility, saveFacilityProgress, sepoliaExplorerTx, short, supplyReserve, wrapSepoliaEth, type CreateFacilityTerms, type FacilityRecord, type TestnetBalances } from "./data/verd";
...
function Facilities() {
  ...
  const [saved] = useState(() => readRecentFacilities());
  const validId = /^0x[a-fA-F0-9]{64}$/.test(facilityId);
  return <Shell><section className="page-head grid"><div><span className="mono">FACILITIES / CREDITCOIN TESTNET</span><h1>Inspect terms before any wallet action.</h1><p>Open a facility by its onchain ID, or return to one you created or continued from this browser.</p></div><Link className="button button-dark" to="/create">Create facility terms</Link></section><section className="facility-recovery"><span className="mono">RETURN TO A FACILITY</span><div><input aria-label="Facility ID" placeholder="Paste a 32-byte facility ID" value={facilityId} onChange={event => setFacilityId(event.target.value.trim())} /><Link className={`button button-dark${validId ? "" : " disabled-link"}`} to={validId ? `/facilities/${facilityId}` : "/facilities"} onClick={event => { if (!validId) event.preventDefault(); else rememberFacility(facilityId); }}>Open facility</Link></div><small>Facility IDs are public onchain records. This browser remembers the facilities you open, but chain state stays authoritative.</small>{saved.length > 0 && <div className="recent-facilities">{saved.map(id => <Link to={`/facilities/${id}`} key={id}><span className="mono">{short(id)}</span><span>Return to facility →</span></Link>)}</div>}</section><section className="summary-strip"><div><span>Recorded principal</span><strong>{facility ? formatCtc(facility.principal) : "Unavailable"}</strong></div><div><span>Preferred rate condition</span><strong>{facility?.preferredRateActive ? "Active" : "Pending"}</strong></div><div><span>Record status</span><strong>{facility ? FACILITY_STATE_LABELS[facility.state] : "Unavailable"}</strong></div></section><section className="facility-list"><div className="table-head mono"><span>FACILITY</span><span>COUNTERPARTY</span><span>PRINCIPAL</span><span>CURRENT APR</span><span>STATE</span><span /></div>{loading && <div className="chain-notice">Reading the recorded deployment from CC3...</div>}{error && <div className="chain-notice chain-error"><span>{error}</span><button className="text-link" onClick={refresh}>Retry</button></div>}{facility && <Link to={`/facilities/${VERIFIED_DEPLOYMENT.facilityId}`} className="facility-row"><span className="facility-id mono">{short(facility.id)}</span><span><strong>{short(facility.borrower)}</strong><small>Borrower</small></span><span>{formatCtc(facility.principal)}</span><span className={facility.preferredRateActive ? "green" : ""}>{formatApr(facility.currentAprBps)}</span><Status tone={facility.preferredRateActive ? "positive" : "neutral"}>{FACILITY_STATE_LABELS[facility.state]}</Status><span className="row-action">Open facility record →</span></Link>}</section></Shell>;
}
...
function CreateFacility() {
  ...
  const next = () => {
    ...
    if (step === 2 && new Date(terms.qualificationDeadline) > new Date(terms.maturity)) return setFormError("The qualification deadline must be on or before maturity.");
    if (step === 2 && new Date(terms.qualificationDeadline).getTime() < Date.now() + 10 * 60 * 1000) return setFormError("Leave at least ten minutes for the reserve qualification.");
    if (step === 2 && new Date(terms.maturity).getTime() < new Date(terms.qualificationDeadline).getTime() + 15 * 60 * 1000) return setFormError("Leave at least fifteen minutes between qualification and maturity.");
  ...
  const submit = async () => {
    ...
    setPending(true);
    try { const created = await createFacility(terms); rememberFacility(created.facilityId); setResult(created); }
  ...
  return <Shell><section className="create-shell"><aside className="create-aside"><span className="mono">CREATE FACILITY</span><h1>Set clear terms for working capital.</h1><ol>{["Counterparties", "Financial terms", "Condition and review"].map((item, index) => <li className={step === index + 1 ? "active" : step > index + 1 ? "done" : ""} key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</li>)}</ol></aside><section className="form-panel">
    ...
    {step === 2 && <><h2>Set the financial terms.</h2><p className="lead">Principal is native tCTC. APR entries are percentages. Leave at least ten minutes to qualify and fifteen more minutes before maturity.</p><div className="field-grid"><Field label="Principal" name="principal" type="number" value={terms.principal} onChange={update} suffix="tCTC" /><Field label="Standard APR" name="standardApr" type="number" value={terms.standardApr} onChange={update} suffix="%" /><Field label="Preferred APR" name="preferredApr" type="number" value={terms.preferredApr} onChange={update} suffix="%" /><Field label="Maturity" name="maturity" type="datetime-local" value={terms.maturity} onChange={update} /><Field label="Qualification deadline" name="qualificationDeadline" type="datetime-local" value={terms.qualificationDeadline} onChange={update} /></div></>}
    ...
    <div className="form-actions">{step > 1 && !result && <button className="text-link" onClick={() => setStep(step - 1)}>← Back</button>}{!result && <button className="button button-dark" disabled={pending} onClick={step < 3 ? next : submit}>{pending ? "Confirm in wallet" : step < 3 ? "Continue" : !wallet.account ? "Connect wallet" : "Create facility"}</button>}</div>
...
function NextAction({ facility, onRefresh }: { facility?: FacilityRecord; onRefresh?: () => Promise<void> }) {
  ...
  const [sepoliaTransactionHash, setSepoliaTransactionHash] = useState<string>();
  ...
  const [jobStatus, setJobStatus] = useState<{ status: string; nextAction: string; terminal: boolean; lastErrorCategory?: string | null; lastErrorMessage?: string | null }>();
  ...
  const [balances, setBalances] = useState<TestnetBalances>();
  ...
  const activeJob = Boolean(jobId && !jobStatus?.terminal);
  ...
  const canRelease = Boolean(releaseUnlocked && !facility?.reserveReleased && !releaseTransactionHash && !activeJob);
  const releaseSourceReady = Boolean(facility?.repaid && !facility.reserveReleased && /^0x[a-fA-F0-9]{64}$/.test(releaseTransactionHash ?? "") && !activeJob);
  const canRepay = Boolean(facility?.drawn && !facility.repaid && Date.now() / 1000 >= facility.maturity);
  const lenderNeedsCtc = Boolean(facility && !facility.funded && isLender && balances && balances.cc3 <= facility.principal);
  const persist = (patch: Parameters<typeof saveFacilityProgress>[1]) => { if (facility) saveFacilityProgress(facility.id, patch); };
  const refreshBalances = () => {
    if (!wallet.account) return;
    setBalanceError(undefined);
    void readTestnetBalances(wallet.account).then(setBalances).catch(() => setBalanceError("Balances are temporarily unavailable. Check the wallet before signing."));
  };

  useEffect(() => {
    if (!facility) return;
    rememberFacility(facility.id);
    const saved = readFacilityProgress(facility.id);
    if (!saved) return;
    setLockerTransactionHash(saved.lockerTransactionHash);
    setReserveSupplyHash(saved.reserveSupplyHash);
    setReleaseTransactionHash(saved.releaseTransactionHash);
    setSourceTxHash(saved.sourceTxHash ?? "");
    setSourceBlock(saved.sourceBlock ?? "");
    setJobId(saved.jobId);
    setJobOperation(saved.jobOperation);
  }, [facility?.id]);
  useEffect(() => {
    if (!facility) return;
    persist({ lockerTransactionHash, reserveSupplyHash, releaseTransactionHash, sourceTxHash: sourceTxHash || undefined, sourceBlock: sourceBlock || undefined, jobId, jobOperation });
  }, [facility?.id, lockerTransactionHash, reserveSupplyHash, releaseTransactionHash, sourceTxHash, sourceBlock, jobId, jobOperation]);
  useEffect(() => {
  ...
  useEffect(() => { refreshBalances(); }, [wallet.account, facility?.id]);
  useEffect(() => {
    if (!jobId) return;
    let active = true;
    const poll = () => void readQualificationJob(jobId).then(status => {
      if (!active) return;
      setJobStatus(status);
      if (status.terminal) {
        if (status.status === "manual_review") setActionError(status.lastErrorMessage ?? "Verification needs review. Your source transaction is preserved.");
        else void onRefresh?.();
      }
    }).catch(() => undefined);
    poll();
  ...
  const registerJob = async (operation: "binding" | "qualification" | "release") => {
    if (!facility || !/^0x[a-fA-F0-9]{64}$/.test(sourceTxHash)) throw new Error("Enter the confirmed Sepolia transaction hash.");
    const result = await registerQualificationJob({ operation, facilityId: facility.id, sourceTxHash, sourceBlock: sourceBlock ? Number(sourceBlock) : undefined });
    setJobOperation(operation);
    setJobId(result.job?.jobId);
    persist({ jobOperation: operation, jobId: result.job?.jobId });
  };
  const drawStandard = async () => {
    setActionError(undefined);
    if (!wallet.account) { await wallet.connect(); return; }
    if (!isBorrower) { setActionError("Connect the borrower wallet to draw this facility."); return; }
    if (!wallet.correctNetwork) await wallet.switchNetwork();
    setPending(true);
    try { setTransactionHash(await drawFacility(facility!.id)); }
    catch (reason) { setActionError(reason instanceof Error ? reason.message : "The standard-rate draw was not submitted."); }
    finally { setPending(false); }
  };
  const act = async () => {
    setActionError(undefined);
    ...
    if (canRelease) {
      ...
      if (!wallet.sepoliaNetwork) await wallet.switchSepolia();
      setPending(true);
      try { const hash = await releaseReserve(facility!.reserveLocker); setReleaseTransactionHash(hash); setSourceTxHash(hash); setSourceBlock(""); persist({ releaseTransactionHash: hash, sourceTxHash: hash }); }
    ...
    if (needsLocker) {
      ...
      if (!wallet.sepoliaNetwork) await wallet.switchSepolia();
      setPending(true);
      try { const hash = await createReserveLocker(facility!.id, facility!.borrower, facility!.maturity); setLockerTransactionHash(hash); setSourceTxHash(hash); persist({ lockerTransactionHash: hash, sourceTxHash: hash }); }
    ...
    if (canQualify && !reserveSupplyHash) {
      ...
      if (!wallet.sepoliaNetwork) await wallet.switchSepolia();
      const shortfall = facility!.requiredReserveAmount - (balances?.weth ?? 0n);
      if (shortfall > 0n) {
        if (!balances || balances.sepoliaEth < shortfall) { setActionError("This borrower wallet needs more Sepolia ETH and WETH before it can supply the reserve."); return; }
        setPending(true);
        try { setSepoliaTransactionHash(await wrapSepoliaEth(shortfall)); refreshBalances(); }
        catch (reason) { setActionError(reason instanceof Error ? reason.message : "The WETH wrap was not submitted."); }
        finally { setPending(false); }
        return;
      }
      setPending(true);
      try { const hash = await supplyReserve(facility!.reserveLocker, facility!.requiredReserveAmount); setReserveSupplyHash(hash); setSourceTxHash(hash); persist({ reserveSupplyHash: hash, sourceTxHash: hash }); }
    ...
    setPending(true);
    try {
      if (canQualify) await registerJob("qualification");
      else if (canRepay) setTransactionHash(await repayFacility(facility!.id, repaymentSubmissionAmount(facility!)));
      else setTransactionHash(await (facility && !facility.funded ? fundFacility(facility.id, facility.principal) : drawFacility(facility!.id)));
    }
    catch (reason) { setActionError(reason instanceof Error ? reason.message : "The transaction was not submitted."); }
    finally { setPending(false); }
  };
  const details = <>{actionError && <small className="action-message critical">{actionError}</small>}{balanceError && <small className="action-message critical">{balanceError}</small>}{transactionHash && <a className="action-message" href={explorerTx(transactionHash)} target="_blank" rel="noreferrer">Creditcoin transaction submitted · {short(transactionHash)} <Arrow /></a>}{sepoliaTransactionHash && <a className="action-message" href={sepoliaExplorerTx(sepoliaTransactionHash)} target="_blank" rel="noreferrer">Sepolia transaction submitted · {short(sepoliaTransactionHash)} <Arrow /></a>}</>;
  ...
  if (jobId && jobStatus?.status === "manual_review") return <div className="next-action recovery"><div><Status tone="critical">Verification needs review</Status><h2>Your source transaction is preserved.</h2><p>{jobStatus.lastErrorMessage ?? "Verd could not complete verification automatically. Re-read the facility and the evidence before attempting another transaction."}</p><small className="action-message mono">JOB · {short(jobId)} · {jobStatus.nextAction}</small>{details}</div><button className="button button-light" onClick={() => void onRefresh?.()}>Refresh facility</button></div>;
  if (activeJob) return <div className="next-action"><div><Status>Verification in progress</Status><h2>Verd is checking the recorded transaction.</h2><p>The source transaction and job are saved in this browser. The worker continues through the proof and Creditcoin steps while its service is online.</p><small className="action-message mono">JOB · {short(jobId!)} · {jobStatus ? `${jobStatus.status} · ${jobStatus.nextAction}` : "Reconnecting"}</small>{details}</div></div>;
  if (!facility.funded) return <div className="next-action"><div><Status>Funding pending</Status><h2>Fund {formatCtc(facility.principal)} to activate this facility.</h2><p>Only the lender address recorded on the facility can fund it.</p>{wallet.account && <small className="action-message mono">CC3 BALANCE · {balances ? formatCtc(balances.cc3) : "Reading wallet balance"}</small>}{lenderNeedsCtc && <small className="action-message">This wallet needs more testnet tCTC for the principal and gas. <a href="https://docs.creditcoin.org/wallets/using-testnet-faucet" target="_blank" rel="noreferrer">Get testnet tCTC</a>.</small>}{details}</div>{lenderNeedsCtc ? <a className="button button-dark" href="https://docs.creditcoin.org/wallets/using-testnet-faucet" target="_blank" rel="noreferrer">Get testnet tCTC</a> : <button className="button button-dark" disabled={pending || Boolean(transactionHash)} onClick={act}>{transactionHash ? "Transaction submitted" : pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet to fund" : "Fund facility"}</button>}</div>;
  ...
  if (facility.repaid && releaseSourceReady) return <div className="next-action"><div><Status>Release transaction ready</Status><h2>Register the reserve release for verification.</h2><p>The Sepolia release is saved in this browser. Sign once with the borrower wallet so Verd can verify it and record completion on Creditcoin.</p><div className="qualification-fields"><input aria-label="Sepolia reserve release transaction hash" value={sourceTxHash} readOnly /><input aria-label="Source block number" placeholder="Source block (optional)" inputMode="numeric" value={sourceBlock} onChange={event => setSourceBlock(event.target.value)} /></div>{details}</div><button className="button button-dark" disabled={pending} onClick={act}>{pending ? "Sign in wallet" : !wallet.account ? "Connect wallet" : "Register release"}</button></div>;
  if (facility.repaid && canRelease) return <div className="next-action"><div><Status tone="warning">Reserve releasable</Status><h2>Release the reserve on Sepolia.</h2><p>The facility is repaid and the locker has reached its unlock time. Verd will save the transaction locally until it is verified on Creditcoin.</p>{details}</div><button className="button button-dark" disabled={pending} onClick={act}>{pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet" : "Release reserve"}</button></div>;
  ...
  if (needsLocker) return <div className="next-action"><div><Status>Reserve setup</Status><h2>Create the facility reserve locker on Sepolia.</h2><p>The borrower creates one immutable locker before WETH can be supplied and authenticated to this facility. The creation transaction is saved in this browser.</p>{details}</div><button className="button button-dark" disabled={pending || Boolean(lockerTransactionHash)} onClick={act}>{lockerTransactionHash ? "Locker submitted" : pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet" : "Create reserve locker"}</button></div>;
  if (bindingPending) return <div className="next-action"><div><Status>Locker created</Status><h2>Bind the reserve locker to this facility.</h2><p>Verd will verify the factory event and submit the CC3 binding proof automatically.</p><div className="qualification-fields"><input aria-label="Locker creation transaction hash" placeholder="Locker creation transaction hash" value={sourceTxHash} readOnly={Boolean(lockerTransactionHash)} onChange={event => setSourceTxHash(event.target.value)} /><input aria-label="Locker creation block number" placeholder="Source block (optional)" inputMode="numeric" value={sourceBlock} onChange={event => setSourceBlock(event.target.value)} /></div><small className="mono">SEPOLIA LOCKER · {short(sepoliaLocker!)}</small>{details}</div><button className="button button-dark" disabled={pending} onClick={act}>{pending ? "Sign in wallet" : !wallet.account ? "Connect wallet" : "Register locker binding"}</button></div>;
  if (canRepay) return <div className="next-action"><div><Status tone="warning">Repayment due</Status><h2>Repay approximately {formatCtc(estimateRepayment(facility))} to close this facility.</h2><p>Verd adds a small timing buffer for the wallet request. The contract sends any unused testnet amount back to the borrower.</p>{details}</div><button className="button button-dark" disabled={pending || Boolean(transactionHash)} onClick={act}>{transactionHash ? "Transaction submitted" : pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet to repay" : "Repay facility"}</button></div>;
  ...
  if (canQualify) {
    const shortfall = facility.requiredReserveAmount - (balances?.weth ?? 0n);
    const primary = reserveSupplyHash ? "Register qualification" : shortfall > 0n ? balances && balances.sepoliaEth >= shortfall ? `Wrap ${formatToken(shortfall, "ETH")}` : "Get testnet assets" : "Supply reserve";
    return <div className="next-action"><div><Status>{reserveSupplyHash ? "Qualification required" : "Reserve supply"}</Status><h2>{reserveSupplyHash ? "Register the reserve transaction for review." : `Supply ${formatToken(facility.requiredReserveAmount, "WETH")} to the facility locker.`}</h2><p>{reserveSupplyHash ? "The confirmed source transaction is saved in this browser. Sign once with the borrower wallet to start verification." : "The borrower can qualify first or draw at the standard rate. WETH is supplied through the authenticated Sepolia locker."}</p>{wallet.account && <small className="action-message mono">ASSETS · {balances ? `${formatToken(balances.weth, "WETH")} · ${formatToken(balances.sepoliaEth, "ETH")} · ${formatCtc(balances.cc3)}` : "Reading wallet balances"}</small>}{shortfall > 0n && !reserveSupplyHash && <small className="action-message">{balances && balances.sepoliaEth < shortfall ? <>Need {formatToken(shortfall, "ETH")} more on Sepolia. <a href="https://ethereum.org/developers/docs/networks/" target="_blank" rel="noreferrer">Find a Sepolia faucet</a>.</> : "Verd can wrap the missing Sepolia ETH into WETH for this testnet reserve."}</small>}{reserveSupplyHash && <div className="qualification-fields"><input aria-label="Sepolia source transaction hash" value={sourceTxHash} readOnly /><input aria-label="Source block number" placeholder="Source block (optional)" inputMode="numeric" value={sourceBlock} onChange={event => setSourceBlock(event.target.value)} /></div>}{details}</div><div className="action-stack"><button className="button button-dark" disabled={pending} onClick={act}>{pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet" : primary}</button>{!reserveSupplyHash && <button className="text-link" disabled={pending} onClick={drawStandard}>Draw at standard rate <Arrow /></button>}</div></div>;
  }
  return <div className="next-action"><div><Status tone={facility.preferredRateActive ? "positive" : "neutral"}>{facility.preferredRateActive ? "Preferred rate active" : "Standard rate available"}</Status><h2>{formatCtc(facility.principal)} is available at {formatApr(facility.currentAprBps)} APR.</h2><p>The facility is funded and the current rate comes directly from Creditcoin contract state.</p>{details}</div><button className="button button-dark" disabled={pending || Boolean(transactionHash)} onClick={act}>{transactionHash ? "Transaction submitted" : pending ? "Confirm in wallet" : !wallet.account ? "Connect wallet to draw" : "Draw facility"}</button></div>;
}
function EvidenceDrawer({ close, facility }: { close: () => void; facility?: FacilityRecord }) {
  const hasPublishedReference = facility?.id.toLowerCase() === PUBLISHED_COMPLETED_REFERENCE.facilityId.toLowerCase()
    && VERIFIED_DEPLOYMENT.verdAddress.toLowerCase() === PUBLISHED_COMPLETED_REFERENCE.verdAddress.toLowerCase();
