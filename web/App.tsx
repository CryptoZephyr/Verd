...
function Footer() {
  return <footer><div className="wordmark"><LogoMark /> <span>Verd</span></div><p>Fixed-term credit with a reserve-based preferred rate.</p><div><Link to="/docs">Read the docs</Link><Link to="/privacy">Privacy</Link><Link to="/terms">Terms and conditions</Link><a href="https://github.com/CryptoZephyr/Verd">GitHub <Arrow /></a></div><small>VERD · CREDITCOIN · 2026</small></footer>;
}
...
function Home() {
  ...
  return <Shell>
    ...
    <section className="role-paths"><div className="section-head"><span className="mono">CHOOSE YOUR PATH</span><Link className="text-link" to="/docs/start/roles">Compare roles <Arrow /></Link></div><div className="role-grid"><article id="for-lenders"><span className="mono">FOR LENDERS</span><h2>Set the price of working capital.</h2><p>Create a facility with clear principal, rates, reserve condition, qualification deadline, and maturity. Fund only after you have reviewed the terms.</p><Link className="text-link" to="/create">Open a facility <Arrow /></Link></article><article id="for-borrowers"><span className="mono">FOR BORROWERS</span><h2>Review terms before you lock a reserve.</h2><p>Compare the standard and preferred rate, confirm the WETH requirement, then follow the facility's current valid action.</p><Link className="text-link" to="/docs/use/borrower-journey">Follow the borrower journey <Arrow /></Link></article></div></section>
    <section className="reserve-boundary"><div><span className="mono">RESERVE BOUNDARY</span><h2>The reserve qualifies the rate. It does not automatically prove repayment.</h2></div><p>The current testnet ReserveLocker releases on time and with borrower authorization. It does not receive a repayment signal from Creditcoin, so Verd presents repayment and reserve release as separate, inspectable steps.</p><Link className="text-link" to="/docs/start/reserve-boundary">Understand the reserve <Arrow /></Link></section>
    <section className="featured"><div className="section-head"><span className="mono">RECORDED TESTNET FACILITY</span><Link className="text-link" to="/facilities">Inspect current state <Arrow /></Link></div><p className="section-note">One recorded testnet facility for inspecting terms, condition state, and evidence. It is not an offer or a portfolio view.</p><Link to="/facilities/0042" className="featured-row"><strong>Working Capital Facility</strong><span><small>PRINCIPAL</small>{facility ? formatCtc(facility.principal) : "Reading CC3"}</span><span><small>CURRENT APR</small><b className="green">{facility ? formatApr(facility.currentAprBps) : "Unavailable"}</b></span><Status tone={facility?.preferredRateActive ? "positive" : "neutral"}>{facility ? FACILITY_STATE_LABELS[facility.state] : "Loading"}</Status><Arrow /></Link></section>
  </Shell>;
}

function Facilities() {
  const { data: facility, loading, error, refresh } = useFacility();
  return <Shell><section className="page-head grid"><div><span className="mono">RECORDED FACILITY / CREDITCOIN TESTNET</span><h1>Inspect terms before any wallet action.</h1><p>This page shows the current onchain record. Open the workspace to review the rate condition, lifecycle, and evidence for the facility.</p></div><Link className="button button-dark" to="/create">For lenders: open a facility</Link></section><section className="summary-strip"><div><span>Recorded principal</span><strong>{facility ? formatCtc(facility.principal) : "Unavailable"}</strong></div><div><span>Preferred rate condition</span><strong>{facility?.preferredRateActive ? "Active" : "Pending"}</strong></div><div><span>Record status</span><strong>{facility ? FACILITY_STATE_LABELS[facility.state] : "Unavailable"}</strong></div></section><section className="facility-list"><div className="table-head mono"><span>FACILITY</span><span>COUNTERPARTY</span><span>PRINCIPAL</span><span>CURRENT APR</span><span>STATE</span><span /></div>{loading && <div className="chain-notice">Reading the recorded deployment from CC3...</div>}{error && <div className="chain-notice chain-error"><span>{error}</span><button className="text-link" onClick={refresh}>Retry</button></div>}{facility && <Link to="/facilities/0042" className="facility-row"><span className="facility-id mono">{short(facility.id)}</span><span><strong>{short(facility.borrower)}</strong><small>Borrower</small></span><span>{formatCtc(facility.principal)}</span><span className={facility.preferredRateActive ? "green" : ""}>{formatApr(facility.currentAprBps)}</span><Status tone={facility.preferredRateActive ? "positive" : "neutral"}>{FACILITY_STATE_LABELS[facility.state]}</Status><span className="row-action">Inspect facility →</span></Link>}</section></Shell>;
}
...
function Field({ label, name, value, onChange, hint, type = "text", suffix }: { label: string; name: TermsKey; value: string; onChange: (name: TermsKey, value: string) => void; hint?: string; type?: string; suffix?: string }) {
  return <label className="field"><span>{label}</span><div><input name={name} type={type} value={value} onChange={event => onChange(name, event.target.value)} />{suffix && <b>{suffix}</b>}</div>{hint && <small>{hint}</small>}</label>;
}
...
function CreateFacility() {
  ...
  const submit = async () => {
    setFormError(undefined);
    if (!reviewed || !terms.requiredReserve) return setFormError("Enter the WETH reserve and confirm the terms.");
...
function FacilityWorkspace() {
  ...
  const roleLabel = facility ? `${short(facility.borrower)} · Borrower` : "Loading facility details";
  return <Shell>
    ...
    <nav className="workspace-tabs"><a className="active" href="#overview">Overview</a><a href="#activity">Evidence</a><a href="#terms">Terms</a><button onClick={() => setDrawer(true)}>Proofs</button></nav>
...
function NextAction({ facility }: { facility?: FacilityRecord }) {
  ...
  useEffect(() => {
    if (!jobId) return;
    let active = true;
    const poll = () => void readQualificationJob(jobId).then(status => { if (active) setJobStatus(status.terminal ? status.status : `${status.status} · ${status.nextAction}`); }).catch(() => undefined);
    poll();
  ...
  const act = async () => {
    ...
    if (needsLocker) {
      ...
      setPending(true);
      try { setLockerTransactionHash(await createReserveLocker(facility!.id, facility!.borrower, facility!.maturity)); }
    ...
    if (bindingPending) {
      ...
      setPending(true);
      try { const result = await registerQualificationJob({ operation: "binding", facilityId: facility!.id, sourceTxHash, sourceBlock: sourceBlock ? Number(sourceBlock) : undefined }); setJobId(result.job?.jobId); }
  ...
  if (bindingPending) return <div className="next-action"><div><Status>Locker created</Status><h2>Bind the reserve locker to this facility.</h2><p>Register the Sepolia factory transaction with the borrower wallet. The durable worker will verify the factory event and submit the CC3 binding proof.</p><div className="qualification-fields"><input aria-label="Locker creation transaction hash" placeholder="Sepolia locker creation transaction hash" value={sourceTxHash} onChange={event => setSourceTxHash(event.target.value)} /><input aria-label="Locker creation block number" placeholder="Source block (optional)" inputMode="numeric" value={sourceBlock} onChange={event => setSourceBlock(event.target.value)} /></div><small className="mono">SEPOLIA LOCKER · {short(sepoliaLocker!)}</small>{wallet.error && <small className="action-message critical">{wallet.error}</small>}{actionError && <small className="action-message critical">{actionError}</small>}{jobId && <span className="action-message">Binding job registered · {short(jobId)}</span>}{jobStatus && <small className="action-message mono">JOB STATUS · {jobStatus}</small>}</div><button className="button button-dark" disabled={pending || Boolean(jobId)} onClick={act}>{jobId ? "Binding job registered" : pending ? "Sign in wallet" : !wallet.account ? "Connect wallet" : !wallet.correctNetwork ? "Switch to CC3" : "Register locker binding"}</button></div>;
...
function EvidenceDrawer({ close, facility }: { close: () => void; facility?: FacilityRecord }) {
  ...
  return <div className="drawer-wrap" role="dialog" aria-modal="true" aria-label="Facility evidence"><button className="drawer-backdrop" onClick={close} aria-label="Close evidence" /><aside className="drawer"><div className="drawer-head"><div><span className="mono">{short(VERIFIED_DEPLOYMENT.facilityId)}</span><h2>Execution evidence</h2></div><button onClick={close} aria-label="Close">×</button></div><p className="drawer-intro">Real records supporting the verified facility state. Chain state remains authoritative.</p>{items.map(([title,status,tone,main,meta,href]) => <div className="evidence-item" key={title}><div><span className="mono">{title}</span><Status tone={tone}>{status}</Status></div><strong>{main}</strong><small className="mono">{meta}</small>{href ? <a className="text-link" href={href} target="_blank" rel="noreferrer">Inspect record <Arrow /></a> : <span className="evidence-unavailable">No external record available</span>}</div>)}</aside></div>;
}
